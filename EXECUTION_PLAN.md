...
8. **Per-client deployment implementation** — per the signed-off plan's own §7 ordering: provision a test client (`tools/provision-client.ts`) → extend the AI tool-contract with ERP tools → run the first automated workflow ("low-stock → draft purchase order") in shadow mode → onboard the first real client.
   ...
   - **Next action:** write `tools/provision-client.ts` against the revised (connection-string-input) design. Live end-to-end testing needs an actual client-supplied Supabase connection string, which doesn't exist yet (no real client onboarded) — the script itself can be written and unit-tested now; full live provisioning waits for a real or test Supabase project's credentials.
