...
8. **Per-client deployment implementation** — per the signed-off plan's own §7 ordering: provision a test client (`tools/provision-client.ts`) → extend the AI tool-contract with ERP tools → run the first automated workflow ("low-stock → draft purchase order") in shadow mode → onboard the first real client.
   - **Architecture change (2026-09-05):** attempting this surfaced that the working Neon account has `Projects Limit: 0` (cannot create a second project). Rather than work around it, the user made a deliberate ownership decision via brainstorming: **each client creates and owns their own Supabase project**, hands us a connection string; we never provision or own client databases directly. Neon stays as the dev/test database only (`AI CRM-ERP-Promo`, unaffected). Existing RLS/`withTenant()` pattern carries over unchanged (Supabase is standard Postgres). `provision-client.ts`'s spec revised accordingly — see [the plan doc](docs/superpowers/specs/2026-09-03-per-client-deployment-and-ai-automation-plan.md) §2.1/§3.1, commit `3fc7ae9`.
   - **Forward-looking note (not designed, not scheduled):** user flagged that compute (currently Cloudflare Workers) may move to a self-hosted VPS at some future point. Recorded in the plan doc §2.1 so it isn't lost; not part of this item's scope.
   - **`tools/provision-client.ts` built and verified (commit `b724fd5`).** Takes `--client-name`, `--connection-string`, optional `--manifest` (reads the exact shape `dev-tools/client-config.html` generates), `--dry-run`. Runs all 19 migrations in order (with the `app_runtime` password generated in Node and substituted for migration 0001's psql-style variable — no `psql` binary dependency), inserts `tenant` + `tenant_erp_settings` rows, prints `app_runtime` credentials once for manual deploy-secret storage. Idempotent (safe re-run). `tsc --noEmit` clean; dry-run tested twice against the real dev database (privilege check + all-19-migrations discovery confirmed; once with no manifest, once with a manifest file matching the dev-page tool's exact output shape) — both succeeded with zero writes.
   - **Not yet done, explicitly out of scope for this step:** actually creating a Cloudflare Worker, attaching a domain, and `tools/fleet-status.ts` (no fleet registry exists yet) — per the script's own docstring, these remain manual/future work. **Full live provisioning against a real client's Supabase project is still blocked** — no actual client has signed up for Supabase and handed over a connection string yet, so there's nothing genuine to provision against beyond the dev-DB dry-runs already done.
   - **Also not yet done:** §7 next-steps 3-4 (extend the AI tool-contract with ERP tools; build the first automated workflow in shadow mode) and step 5 (onboard the first real client) — these are separate, larger pieces of work than the provisioning script itself.
